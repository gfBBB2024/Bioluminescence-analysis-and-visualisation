...
file_path = "/Users/fg221/OneDrive - Imperial College London/PhD Milestone/PhD thesis/PhD thesis_Feng Gao/Engineering a Brighter Bacterial Bioluminescence Pathway/dna5861_dna5298_comparison_BY-2.xlsx"
...
# and a single ×10^6 scale indicator at the top
ax.set_yscale("log")
...
# divide the actual values by 1e6 so that only the numeric part
...
lo = np.ceil((ymin / 1e6) * 5) / 5    # step size = 0.2
hi = np.floor((ymax / 1e6) * 5) / 5
...
if len(tick_vals) < 2:
    mid = (ymin * ymax) ** 0.5
    tick_vals = np.array([ymin / 1e6, mid / 1e6, ymax / 1e6])

# Apply the tick positions (convert back to absolute values)
ax.set_yticks(tick_vals * 1e6)

# Add a single ×10^6 label at the top-left of the y-axis
ax.text(
    -0.12, 1.02,
    r"$\times 10^6$",
